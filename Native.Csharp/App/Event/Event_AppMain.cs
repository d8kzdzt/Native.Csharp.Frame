...
namespace Native.Csharp.App.Event
{
	public static class Event_AppMain
	{
		...
		public static void Registbackcall (ContainerBuilder container)
		{
			...
			// 注入 Type=1001 的回调
			container.RegisterType<Event_CqStartup>().Keyed<ICqStartup>("酷Q启动事件");
			// 注入 Type=1002 的回调
			container.RegisterType<Event_CqExit>().Keyed<ICqExit>("酷Q关闭事件");
			// 注入 Type=1003 的回调
			container.RegisterType<Event_CqAppEnable>().Keyed<ICqAppEnable>("应用已被启用");
			// 注入 Type=1004 的回调
			container.RegisterType<Event_CqAppDisable>().Keyed<ICqAppDisable>("应用将被停用");
			// 注入 Type=2 的回调
			container.RegisterType<Event_GroupMessage>().Keyed<IReceiveGroupMessage>("群消息处理");
		}
		...
		public static void Resolvebackcall (ContainerBuilder container)
		{
			// 此方法的参数 container 是于插件加载时初始化好的反向注入容器 (IOC 容器)
